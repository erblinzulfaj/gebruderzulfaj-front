<template>
  <div class="products-page">
    ...
    <div
      ...
      v-if="selectedItem && filteredProducts.length"
    >
        ...
        <tbody>
          <tr v-for="product in filteredProducts" :key="product.id">
            ...
            <td class="tdd" style="width: 30%;">
              ...
              />
              <button
                @click="incrementQuantity(product.id, product.newQuantity)"
                class="btn btn-success me-2"
              >
                +
              </button>
              <button
                @click="decrementQuantity(product.id, product.newQuantity)"
    ...
    <div v-else>
      <p style="color: red;">
        {{
          selectedItem
            ? $t("there_are_no_products_of_the_type_you_selected")
            : $t("you_did_not_click_the_product_type")
        }}
      </p>
    </div>
    <hr />
...
const incrementQuantity = async (productId, newQuantity) => {
  try {
    ...
    if (!isNaN(quantityToAdd) && quantityToAdd > 0) {
      ...
      console.log(
        `Quantity updated successfully for product with ID ${productId}`
      );
